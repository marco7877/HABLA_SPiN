...
do
	...
	do
		...
		part_mag_out=${origin}/${subj}_ses-1_${task}_echo-${n_echo}_part-mag_bold_${method}_dsd
		...
		echo "addpath(genpath('/bcbl/home/public/MarcoMotion/toolboxes/')); cd '${repo}'; ARG.temporal_phase=1; ARG.phase_filter_width=10; ARG.save_add_info=1; ARG.DIROUT='${output}'; ARG.noise_volume_last=3; NIFTI_NORDIC ('${part_mag}.nii.gz','${part_phase}.nii.gz','${part_mag_out}',ARG)" > $temp_nordic
	...
	volumes=$(3dinfo -nvi ${part_mag_out}.nii)
